...
def to_pickle1(visual, it, keys, length):
    ...
    lst = []
    for key in keys:
        lst.append([key[length:], visual[key][0]])
    # pd.DataFrame(lst, columns=['pid', 'visual']).to_pickle('../data/visual_feature/train_visual_feature_{}.pkl'.format(it))
...
def to_pickle2(visual, it, keys, length):
    ...
    for key in keys:
        lst.append([key[length:], visual[key][0]])
    # pd.DataFrame(lst, columns=['pid', 'visual']).to_pickle('../data/visual_feature/test_visual_feature_{}.pkl'.format(it))
    return pd.DataFrame(lst, columns=['key', 'visual'])
...
def proc_visual(visual, length, yield_keys, to_pickle, name):
    lst = Parallel(n_jobs=-1, backend='multiprocessing')(
    ...
    lst.to_pickle(
        '../data/visual_feature/visual_feature_{}.pkl'.format(name))
    print(name, 'finished...')

if __name__== "__main__":
    # train_visual = np.load('../data/train_visual.zip')
    # test_visual = np.load('../data/test_visual.zip')
    train_visual = '../data/train_visual.zip'
    test_visual = '../data/test_visual.zip'

    train_str_length = len('preliminary_visual_train/')
    test_str_length = len('preliminary_visual_test/')
