...
class Model(object):
    ...
    def _create_weights(self):
        ...
        self.W_usr_like_lda_face = tf.get_variable(shape=[self.dim_usr_like_lda_face, self.dim_k],
                                              initializer=tf.glorot_uniform_initializer(),
                                              dtype=tf.float32, name='W_usr_like_lda_face')
        ...
        with tf.name_scope('user_express'):
            ...
            self.Usr_Like_VAE = tf.nn.embedding_lookup(self.user_like_vae,
                                                       tf.cast(self.user_indices,
                                                               tf.int32))  # [batch_size, dim_usr_like_vae]

            self.Usr_Like_LDA_FACE = tf.nn.embedding_lookup(self.user_like_lda_face,
                                                            tf.cast(self.user_indices,
                                                                    tf.int32))  # [batch_size, dim_lda_face]
            ...
            self.bias_user_like_lda_face = tf.get_variable(shape=[self.dim_k], initializer=tf.zeros_initializer(),
                                                     dtype=tf.float32,
                                                     name='bias_user_like_lda_face')
            self.Usr_Like_LDA_FACE_Emb = tf.matmul(self.Usr_Like_LDA_FACE,
                                                   self.W_usr_like_lda_face) + self.bias_user_like_lda_face  # [batch_size, dim_k]

            self.Usr_Like_LDA_FACE_Emb = tf.nn.relu(self.Usr_Like_LDA_FACE_Emb)

            self.Usr_Like_VAE_Emb = self.Usr_Like_VAE_Emb + self.Usr_Like_LDA_FACE_Emb
    ...
    def _create_loss(self):
        ...
        self.biases = [self.bias, self.bu,
                       self.c_oh_Att, self.b_oh_Att, self.b_in_prd_att, self.bias_usr_like_vae, self.bias_user_like_lda_face,
        ...
        self.params = [self.Usr_Emb, self.Wwords_Emb, self.W_Ctx, self.W_usr_feat_emb, self.W_usr_like_vae,
                       self.W_usr_like_lda_face, self.W_LDA_emb, ] + self.I_One_hot_a + self.biases
    ...
    def _save_preds(self, test_data, preds, save_path):
        ...
        test_data[['uid', 'pid', 'preds']].to_pickle(
            os.path.join(save_path, 'likevisual2048_likeldaface_decay_reg002_visual_lr0012.pkl'))
...
if __name__ == '__main__':
    ...
    user_like_data = pd.read_pickle('../model/user_like_visual_lda_face.pkl')
    user_like_data = user_like_data.sort_values(['user_indices'])
    user_like = np.array(user_like_data['user_like_visual'].tolist(), np.float32)
    ...
    user_like_lda_face = np.concatenate([user_like_data['user_like_lda'].tolist(), user_like_data['user_like_face'].tolist()],
    ...
    model_params = {
        ...
        'user_like_lda_face': user_like_lda_face,
        'dim_lda': 6,
        'lr': 0.0012,
        'prefix': None,
        'seed': 1024,
        ...
        'checkpoint_path': '../model/likeldaface.mdl'
    }
